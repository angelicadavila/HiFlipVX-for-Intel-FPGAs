...
#pragma HLS data_pack variable=output2
	...
	std::string input_bmp_filename    = "rabbit_grey.bmp";
	std::string input1_bmp_filename    = "in_test.bmp";
	std::string output_bmp_filename   = "out_test.bmp";

	//Avalon master Interfaces
	vx_image<filter_u_type, 1> tmp_in[COLS_FHD*ROWS_FHD];
	vx_image<filter_s_type, 1> tmp_out[COLS_FHD*ROWS_FHD];
	//vx_image<filter_s_type, VEC_NUM> tmp_out1[COLS_FHD*ROWS_FHD/VEC_NUM];

	#ifdef Xilinx
    
	#elif Intel
	DramTypeIn0 mm_in(tmp_in,sizeof(vx_image<filter_u_type,1>)*COLS_FHD*ROWS_FHD);
	DramTypeOut0 mm_out(tmp_out,sizeof(vx_image<filter_u_type,1>)*COLS_FHD*ROWS_FHD);
	...
	vx_image<filter_u_type, 1> data;
	for (int i = 0; i < (ROWS_FHD*COLS_FHD); ++i) {
		data= static_cast<filter_u_type>( in_img[i]);
		tmp_in[i]=data;
	}

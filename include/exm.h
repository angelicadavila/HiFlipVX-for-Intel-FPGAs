...
void ImgSobel(_vx_in(vx_image<SrcType, VEC_NUM>) input0  _vx_vecsize((HEIGHT*WIDTH)/VEC_NUM), 
	      _vx_out(vx_image<DstType, VEC_NUM>) output1 _vx_vecsize((HEIGHT*WIDTH)/VEC_NUM), 
              _vx_out(vx_image<DstType, VEC_NUM>) output2 _vx_vecsize((HEIGHT*WIDTH)/VEC_NUM) )

{
	HwSobel<SrcType, DstType, VECTOR_PIXELS(HEIGHT, WIDTH, VEC_NUM), VEC_NUM, WIDTH, HEIGHT, KERN_SIZE, BORDER_TYPE>(input0, output1, output2);
}

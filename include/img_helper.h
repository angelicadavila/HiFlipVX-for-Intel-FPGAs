...
*
* Update 01-2020
* To support Intel FPGA Devices, some changes and macros are added
* - The unrolling pragma was redifined due the compiliers uses in differents lines of for-loop
* - Intel requires special types to communicate with function in the pipe and I/O
*   	vx_image identifies with vx_streamIntel_e this three types
*   	to be compatible con original syntax of OpenVX the default vaule is the pixel structure
*
*/
...
/******************************************************************************/
/* Update to support Intel FPGA devices*/
/******************************************************************************/

/*! \brief Enumeration to support Intel Streammings
*/
enum vx_streamIntel_e {
	vx_streamIn_e,  ///< Streaming input interface to the Intel component
	vx_streamOut_e, ///< Streaming output interface to the Intel component
	vx_stream_e, 		///< Streaming interface to the Intel component
	vx_streamNull_e ///< defaut value for no-streamming
};


#ifdef Xilinx
//In Xillinx the stream parameter is ignored
template<class TYPE, const size_t SIZE, int stream_type = vx_streamNull_e>
using vx_image= typename vx_image_t<TYPE, SIZE>::type;

template<class TYPE, const size_t SIZE, int stream_type = vx_streamNull_e>
using vx_distribution= typename vx_image_t<TYPE, SIZE>::type;
#elif Intel
template<bool B, class T, class F>
struct conditional { typedef T type; }; ///< implementing data-type functionalitie
...
/*! \brief Intel SDK defines 3 stream access, if an embeded system need to use the lybrary
		   ...
		   If the streams its to communicated funtions, the defaul is stream.
		   read more about streammings: Intel® High Level Synthesis Compiler Pro Editio, Reference manual 19.4
		   Default: vx_image
@param  TYPE	 Data type
@param  SIZE     Vector size
@param  stream_type     type of stream
@param  buff_capacity   size of the buffer streamming
*/
...
/*! \brief Intel SDK defines 3 stream access, if an embeded system need to use the lybrary
		   the stream_in and stream_out are required,.
		   Default: vx_distribution
@param  TYPE	 Data type
@param  SIZE     Vector size
@param  stream_type     type of stream
@param  buff_capacity   size of the buffer streamming
*/
template<class TYPE, const size_t SIZE,int stream_type = vx_streamNull_e, uint buff_capacity=64>
using vx_distribution= typename conditional<stream_type == vx_streamIn_e,
									ihc::stream_in<TYPE>,
		typename conditional<stream_type==vx_streamOut_e, 
									ihc::stream_out<TYPE>,
		typename conditional<stream_type==vx_stream_e, 
									ihc::stream<TYPE,ihc::buffer<buff_capacity>>,
...
//To redefine the pragmas used by each manufacturer using a macro.
#ifdef Xilinx
        #define UNROLL_XILINX() _Pragma("HLS unroll")
        #define UNROLL_INTEL()
#elif Intel
        #define UNROLL_INTEL() _Pragma("unroll")
...
*/
template<typename ImageType, typename PixelType, vx_uint32 COLS, vx_uint32 ROWS, int VEC_NUM = 1>
...
#endif
#elif Intel
	return (ImageType *)(new PixelType[COLS*ROWS/VEC_NUM]);
#endif
}
...
namespace HIFLIPVX {
	...
	*/
	enum FilterOperation {
		...
		NON_MAX_SUPPRESSION_PHASE4,  /*!< \brief Do Non-Maxima Suppression */
        CENSUS_TRANSFORM 	 /*!< \brief Census transform (optimized for kernel structure) */
	};
	...
	*/
	enum PixelwiseOperationA {
		...
		PHASE4,					/*!< \brief Implements the Phase for 4 direction, avoiding arctan*/
...
UNROLL_INTEL()
		for (vx_uint8 j = 0; j < KERN_SIZE; j++) {
UNROLL_XILINX()
			kernel_sum += static_cast<vx_uint64>(std::abs(static_cast<vx_int64>(kernel[i][j])));
		}
	}
...
UNROLL_INTEL()
	for (vx_uint8 i = 0; i < KERN_SIZE; i++) {
UNROLL_XILINX()
		kernel_sum += static_cast<vx_uint64>(std::abs(static_cast<vx_int64>(kernel[i])));
	}

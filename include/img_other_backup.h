...
template <typename SrcType, typename DstType, vx_uint16 VEC_NUM, const vx_uint32 
	IMG_PIXELS, vx_df_image_e INPUT_TYPE, vx_df_image_e OUTPUT_TYPE,int 
	stream_type0 = vx_stream_e, int stream_type1 = vx_stream_e>
 #ifdef Xilinx
void ConvertColor(SrcType input[IMG_PIXELS], DstType output[IMG_PIXELS]) {
 #elif Intel
void ConvertColor(vx_image<SrcType, VEC_NUM, stream_type0> &input,
vx_image<DstType, VEC_NUM, stream_type1> &output) {
 #endif
 #ifdef Xilinx
#pragma HLS INLINE
 #endif
...
typename vx_type0 = vx_image<SrcType, 1, vx_stream_e> , 
typename vx_type1 = vx_image<vx_uint8, 1, vx_stream_e>>
...
template<typename DstType, vx_uint16 CHANNEL_AMOUNT, vx_uint32 IMG_PIXELS, 
	...
	typename vx_type0 = vx_image<vx_uint8, 1, vx_stream_e>>
 #ifdef Xilinx
...
vx_type0 &input3, vx_image<DstType,1,vx_stream_e> &output) {
 #endif
 #ifdef Xilinx
#pragma HLS INLINE
 #endif
...
//#pragma ii 1
		...
		#elif Intel
			vx_image<vx_uint8,1> tmp0 = input0.read();
			input_data[0] = tmp0.pixel[0];
 		#endif
		}
		if (CHANNEL_AMOUNT > 1){
 		#ifdef Xilinx
		...
		#elif Intel
			vx_image<vx_uint8,1> tmp1 = input1.read();
			input_data[1] = tmp1.pixel[0];
 		#endif
		}
		if (CHANNEL_AMOUNT > 2){
 		#ifdef Xilinx
		...
		#elif Intel
			vx_image<vx_uint8,1> tmp2 = input2.read();
			input_data[2] = tmp2.pixel[0];
 		#endif
		}
		if (CHANNEL_AMOUNT > 3){
 		#ifdef Xilinx
		...
		#elif Intel
			vx_image<vx_uint8,1> tmp3 = input3.read();
			input_data[3] = tmp3.pixel[0];
 		#endif
		}
		// Convert input to output channel
...
// (vx_int32)floor((x_dst + 0.5f)*(COLS_IN/COLS_OUT) - 0.5f) using
//floating point computation
			vx_uint32 y_src = (((((y_dst << 1) + 1) * ROWS_SCALE) >> ACCURACY) -1) >> 1; 
// (vx_int32)floor((y_dst + 0.5f)*(ROWS_IN/ROWS_OUT) - 0.5f) using
// floating point computation
...
#elif Intel
void EqualizeHist(vx_image<SrcType, VEC_NUM, stream_type0> &input, 
	vx_image<SrcType, VEC_NUM, stream_type1> &output)
 #endif
{
 #ifdef Xilinx
#pragma HLS INLINE
...
* static_cast<vx_uint32>(SCALE)) >> ACCURACY);
   ...
   //cumulative distribuiton
    vx_uint32 cum[DISTRIBUTION_BINS];
    cum [0] = histogram [0];
	for (vx_uint32 i = 1; i < DISTRIBUTION_BINS; i++) {
		cum[i] = histogram [i] + cum [i-1];
	}
   // cdf probabilities
    vx_uint8 cdf[DISTRIBUTION_BINS];
	for (vx_uint32 i = 0; i < DISTRIBUTION_BINS; i++){ 
        cdf[i] = static_cast<vx_uint8>(cdf[i] * DISTRIBUTION_BINS/ IMG_PIXELS );
	} 
	...
	for (vx_uint32 i = 0; i < IMG_PIXELS / VEC_NUM; i++) {
      	auto base_index =  input_Y[i];
		UNROLL_INTEL()
		for (vx_uint8 j = 0; j < VEC_NUM; j++) {
		UNROLL_XILINX()
      	 	vx_uint8 eq_index =  base_index.pixel[j];
      	 	input_data.pixel [j] = cdf [eq_index];   
		}

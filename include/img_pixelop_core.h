...
@return                Result
*/
template<typename ScalarInt, typename ScalarUint, typename CompInt, 
typename CompUint, vx_convert_policy_e CONV_POLICY, const vx_round_policy_e ROUND_POLICY>
...
/******************************************************************************/
/* Compute Pixel-wise Phase */
/******************************************************************************/

/*! \brief Compute Phase of one pixel (signed) 
@param ScalarInt      Data type of the image pixels
@param CompUint        Data type for the computation
@param CONV_POLICY     Conversion policy (wrap & saturate)
@param ROUND_POLICY    Rounding policy (to zero & nearest even)
...
ScalarInt Phase4Signed(ScalarInt input1, ScalarInt input2) {
 #ifdef Xilinx 
#pragma HLS INLINE 
 #endif  
	...
	CompInt A = static_cast<CompInt>(input1);
	CompInt B = static_cast<CompInt>(input2);
	float C = static_cast<float>(A/(float)B);

	//compute 4 phase regions. Horizontal =1, Diag. Pos=2, Diag. Neg =3, Vertical = 0
	//https://livrepository.liverpool.ac.uk/3028893/1/200832211_Jan2018.pdf
    CompInt D = static_cast<CompInt> (2 * fabs(A));	
    B = ( fabs(B));	
...
#pragma HLS INLINE 
	...
	//PHASE
	} else if (OPERATION_TYPE == HIFLIPVX::PHASE4) {		
		if (SCALAR_TYPE == VX_TYPE_INT8)
			result = Phase4Signed<ScalarType, vx_uint8, vx_int8, vx_uint16, CONV_POLICY, ROUND_POLICY>(input1, input2);
...
(OPERATION_TYPE == HIFLIPVX::ARITHMETIC_ADDITION) || (OPERATION_TYPE ==
HIFLIPVX::ARITHMETIC_SUBTRACTION) || (OPERATION_TYPE == HIFLIPVX::MULTIPLY) ||
(OPERATION_TYPE == HIFLIPVX::MAGNITUDE) || (OPERATION_TYPE == HIFLIPVX::PHASE4))
...
enable_if_t<!std::is_same<vx_image<ScalarType,VEC_SIZE>, ScalarType>::value,int> = 0 
>
#ifdef Xilinx 
...
#elif Intel
void PixelwiseSameType(vx_type0 &input1, vx_type1 &input2, vx_type2 &output) {
#endif
 #ifdef Xilinx 
#pragma HLS INLINE 
 #endif  
...
UNROLL_XILINX()
			PixelwiseComputeSameType<ScalarType, OPERATION_TYPE, CONV_POLICY, 
				ROUND_POLICY, SCALE>(src1.pixel[j], src2.pixel[j], dst.pixel[j]);
		}
...
/*******/
//Template especialization for Scalar types in Intel with vectorization factor of 1
template<typename ScalarType, vx_uint8 VEC_SIZE, vx_uint32 IMG_PIXEL, HIFLIPVX::PixelwiseOperationA OPERATION_TYPE, vx_convert_policy_e CONV_POLICY, vx_round_policy_e ROUND_POLICY, vx_uint32 SCALE,typename vx_type0, typename vx_type1,typename vx_type2, enable_if_t<std::is_same<vx_image<ScalarType,VEC_SIZE>, ScalarType>::value,int> = 0>
void PixelwiseSameType(vx_type0 &input1, vx_type1 &input2, vx_type2 &output) {
	// Variables
	vx_image<ScalarType, VEC_SIZE> src1, src2, dst = { 0 };
	// Check if input correct
